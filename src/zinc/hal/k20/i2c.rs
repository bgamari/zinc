...
pub static mut i2c0: I2C = I2C {
  lock: MUTEX_INIT,
  state: Shared {value: UnsafeCell{value: State::Idle}, invariant: marker::InvariantType},
  irq: COND_VAR_INIT,
  irq_number: 11,
  reg: &reg::I2C0,
};
...
impl<'a> Context<'a> {
  ...
  pub fn write(&self, Address(addr): Address, buffer: &[u8]) -> Option<Error> {
    ...
    self.i2c.reg.d.set_data(addr << 1);
    {
      ...
      *state = State::Tx(buffer.as_ptr(), buffer.len());
    }
    self.i2c.irq.wait();
    self.finish()
  }
  ...
  pub fn read(&self, Address(addr): Address, buffer: &mut [u8]) -> Option<Error> {
...

    {
      let crit = NoInterrupts::new();
      let mut state = self.i2c.state.borrow(&crit);
      *state = State::RxStart(buffer.as_mut_ptr(), buffer.len());
    }
...
fn irq_handler(i2c: &I2C) {
  ...
  match *state {
    ...
    State::Idle | State::Failed(_) => {}, //unsafe { abort() }, // spurious interrupt
    State::RxStart(_, _) if status.rxak() => {
      // premature nack
      i2c.reg.c1.set_mst(false);      // send stop
      *state = State::Failed(Error::Nack);
      signal();
    },
    State::RxStart(d, rem) => {
      i2c.reg.c1.set_tx(false);       // we are now receiving
      let _ = i2c.reg.d.get().data(); // throw away byte
      *state = State::Rx(d, rem);
    },
    State::Rx(d, rem) => {
      match rem {
        1 => {
        ...
        _ => {
          ...
          unsafe {
            ...
            *state = State::Rx(d.offset(1), rem-1);
          }
        }
      }
    },
    State::Tx(_, _) if status.rxak() => {
      ...
      unsafe {
        match rem {
          ...
          _ => {
            ...
            *state = State::Tx(d.offset(1), rem-1)
          }
        }
      }
    },
